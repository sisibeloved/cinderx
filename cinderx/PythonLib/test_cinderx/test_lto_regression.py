...
class TestLTOSymbolIntegrity(unittest.TestCase):
    ...
    def test_jitrt_symbols_exist(self):
        ...
        result = subprocess.run(
            ["nm", "-C", str(so_path)], capture_output=True, text=True
        )
        ...
        for func in key_functions:
            with self.subTest(function=func):
                self.assertIn(func, result.stdout, f"{func} not found in symbol table")
    ...
    def test_jitrt_symbols_are_global(self):
        ...
        so_path = self._get_cinderx_so_path()
        result = subprocess.run(["nm", str(so_path)], capture_output=True, text=True)
        self.assertEqual(result.returncode, 0)
            ...
            if "JITRT_" in line:
                ...
                if len(parts) >= 2:
                    ...
                    if sym_type == "t":  # local text symbol
                        func_name = parts[-1]
                        self.fail(
                            f"JITRT function {func_name} is local (t) not global (T)"
                        )
...
class TestLTOPerformanceBaseline(unittest.TestCase):
    ...
    def test_jit_compile_time_reasonable(self):
        ...
        self.assertLess(
            elapsed, 5.0, f"JIT compilation took {elapsed:.2f}s, expected < 5s"
        )
...
class TestLTODetectionAPI(unittest.TestCase):
    ...
    def test_is_lto_enabled_consistent(self):
        ...
        self.assertEqual(
            len(set(results)), 1, "is_lto_enabled() returned inconsistent values"
        )
