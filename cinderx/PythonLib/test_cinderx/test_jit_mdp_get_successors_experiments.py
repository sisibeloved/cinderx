...
class MdpGetSuccessorsExperimentTests(unittest.TestCase):
    def test_get_successors_whole_helper_reduces_exception_deopts(self) -> None:
        code = textwrap.dedent(
            ...
            statep = (1, (charhalf, starhalf, 0), "Super Potion")
            ...
            for _ in range(2000):
                battle = mod.Battle()
                result = battle.getSuccessors(statep)
                total += len(result)
                summary = [(repr(key), value) for key, value in result]
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            env_optimized["PYTHONJIT_ARM_MDP_GET_SUCCESSORS_WHOLE_HELPER"] = "1"
            ...
            self.assertGreaterEqual(o_call_static, b_call_static + 1, (proc_baseline.stdout, proc_optimized.stdout))
            self.assertGreaterEqual(o_check_exc, 1, proc_optimized.stdout)
